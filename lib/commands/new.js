...
module.exports = {
  ...
  generateTemplate(argv) {
    if ( fs.existsSync(argv.filePath) ) {
      console.log('  ✅', `${argv.name}.js function file already exists`)
    } else {
      const tmplFile  = path.join(__dirname, '../templates/function.js.ejs'),
            template  = fs.readFileSync(tmplFile, { encoding: 'utf8' }),
            content   = ejs.render(template, argv);

      fs.writeFileSync(argv.filePath, content)
      console.log('  ✅', `${argv.name}.js function file created`)
    }
  }
}
